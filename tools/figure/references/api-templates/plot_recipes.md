...
|------|------|-------------|---------|-----------|
| **① 原始数据可视化** | 理解数据特征，发现潜在规律 | 直方图、箱线图、密度图、热力图、散点图矩阵、折线图、堆叠柱状图 | 分析数据时按需绘制 | 选择有实际信息的分布、关系或异常视角 |
| **② 中间产物可视化** | 展示求解过程，验证模型行为 | 收敛曲线、分组柱状图、灵敏度折线、残差图、QQ图、混淆矩阵热力图、学习曲线 | 验证模型时按需绘制 | 选择算法适用的收敛、对比或误差分析 |
| **③ 最终结果可视化** | 支撑核心结论，论文直接使用 | 对比柱状图、预测折线+CI、雷达图、气泡图、组合图、地图热力图 | 求解完成后按需绘制 | 选择支撑主张的图；数值表足够时不强制增加图 |

**全局原则**：图数和种类由论点、数据及有来源的适用规则决定，不设统一配额，详见 `chart_selection.md` §图型多样性。
