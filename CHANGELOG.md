...
- 通用 Skill 同步 DSH 配置能力：通过分组提问确定执行范围、材料、论文格式、五类绘图扩展和七类可选协作；沿用已保存偏好，单项执行不重置项目。
- 通用入口提供环境与依赖检测、安装命令和 Agent 安装提示，支持查询运行、证据与快照并预览恢复。
- main 整理为通用 Skill 源码；Release 仅提供通用 Skill ZIP 与 DSH 插件 TGZ，README 改为功能、安装和使用说明。开发实现与验证记录保留在维护者本地。

- 看板右上角增加“开始”入口，支持完整流程、单个阶段和阶段内单项；任务选项读取当前项目已保存的阶段清单，保持 DSH 主题、窄侧栏布局与按需显现的滚动条。
- 确认后通过官方 SessionController 向看板所属会话提交执行指令，沿用该会话的模型、工具和权限；反馈为“已提交”，实际进展在会话和项目状态中查看。
- 启动校验所属会话、项目身份、目录和状态版本；拦截忙碌会话和重复提交。单阶段/单项不重置项目、不放宽原范围，不绕过前置依赖或独立审核。
- 保留 2.1.3 的自动初始化、真实错误和重试修复；新增任务选择与官方宿主提交回归，CI 要求宿主启动检查实际执行、零跳过。

## 2.1.3 - 2026-09-22（本地开发初始化修复）

- 修复工作区沙箱授权失败后看板误提示“请选择预设”的问题，保留实际初始化错误与重试入口；目录权限失败不改变已选预设。
- 新增 Windows 工作区准备工具，默认只读；显式指定父目录、`--apply` 与新备份文件后，只为未来一级项目目录准备 CREATOR OWNER 的 `WRITE_OWNER` 继承，并精确处理符合条件的现有一级项目根，不递归修改深层目录或文件。
- 新增真实官方 Windows fs/shell/sandboxPolicy 下的自动初始化回归，覆盖授权失败、显式准备后重试、两个新建兄弟项目及再次进入的项目身份稳定；Windows CI 要求实际执行且零跳过。
- 权限工具按 Windows 原生 SID 语义比较身份，兼容内置管理员在 SDDL 中的 `LA` 等缩写，精确回滚也识别别名与数值 SID；保存的原始权限描述符不重写。新增三项 SID 回归后，权限专项 **15/15** 通过（0.587 秒），真实沙箱初始化专项再次通过（11.789 秒）。管理员 CI 夹具显式设置目录所有者与继承边界，临时禁用测试进程的所有权旁路特权并在结束后恢复。
- 本机 Python 原有 314 项与新增权限准备 12 项分别全部通过（59.746 秒、0.602 秒），DSH 完整 **33/33** 通过（35.7011 秒），均无失败或跳过；2.1.3 独立 wheel 校验通过。实际 DSH 页面中，原失败空项目及准备后新建的另一个空项目均通过预设与侧栏入口自动初始化，保持 `workspace-write`，没有模型请求。记录为 `project-review/logs/fix-2.1.3-*`。

## 2.1.2 - 2026-09-22（本地开发兼容修复）

- 修复 Python 3.13 在 Windows DSH 受限沙箱中的临时目录权限兼容问题，避免环境检测因探测目录无法访问而把依赖统一显示为异常。
- 检测继续通过宿主授权的沙箱执行，保留隔离导入、超时、输出过滤和只读边界；不因检测失败自动安装依赖或扩大权限。
- 新增实际 Windows 受限沙箱回归，并接入 Node 24、Python 3.13 的宿主 CI，强制执行且拒绝静默跳过；保留 2.1.1 的预设服务隔离修复。
- 本机完整回归 Python **314/314**（56.807 秒）、DSH **31/31**（37.3497 秒）通过，无失败、无跳过；独立 wheel 的资源、三个 profile、材料与配置验证通过。日志为 `project-review/logs/fix-2.1.2-python.log`、`fix-2.1.2-dsh.log`，宿主与浏览器用例边界见测试说明。

## 2.1.1 - 2026-09-22（本地开发修复版本）

- 修复数学建模预设发布 `mathModelWorkbench` 服务时缺少隔离域的问题。原声明触发官方 `agent-preset/invalid`，阻止新建会话并使侧栏入口不可用；现在在 workbench 插件节点声明 `isolate.mathModelWorkbench: true`，由官方 `serviceFor` 按所属预设读取能力。
- 升级时需同时检查本机 profile 的 `cordis.patch.yml`：若其中保存了旧的预设 `config.plugins` 覆盖列表，须在该列表的 workbench 节点同步补齐隔离声明，保留其他自定义项。只替换 npm 包不能修正仍然生效的旧覆盖；随后重启宿主加载新声明。
- 新增真实官方 Loader、PresetRegistry 与 AgentLoop 回归：无隔离声明时复现相同错误且不发布失败会话；修复后创建两个同预设数学会话、一个改名数学预设会话及一个普通会话，验证工具、能力与侧栏识别隔离。DSH 全套 **30/30 通过，0 失败、0 跳过，31.6804 秒**；日志为 `project-review/logs/fix-2.1.1-dsh.log`。此结果不代表整个桌面 GUI 或真实模型对话验收完成。

## 2.1.0 - 2026-09-22（本地开发版本）

- 配置页加入本机环境检测，区分基础必需、当前配置需要和可选依赖，提供复制安装命令与 Agent 安装 Prompt；通过实际执行环境检查，不自动安装。

- 选择 Workbench 预设后可从原生侧栏进入并自动初始化；按会话投影和 composition 能力识别预设，支持改名，幂等复用已建项目。
- 新增材料导入、论文要求与项目配置页；分块上传、输入原件/文本哈希、来源与解析状态、上下文注入和配置变更审查失效。
- 增加并列的数据预处理与模型验证目录：无泄漏 Pipeline、分组/时间验证、敏感性、不确定性、求解状态及机理固定检查。
- 新增六类模块化算法卡片与可执行例程；每子问题最多两个独立模型体系，同物理机理近似按一个模型族。
- 集成固定来源版本的 scientific-visualization、matplotlib、seaborn、scientific-schematics Skill；SciencePlots 可选，drawio 可编辑源文件生成，SciVisAgentSkills 按任务固定哈希加载。
- 七类额外 Subagent 协作默认关闭，可按项目保存选择；阶段独立质检规则保持适用。
- 新版六页看板使用 DSH 原生主题与简洁圆角控件，无 slogan，悬停说明与隐藏滚动条覆盖新材料/设置页面。

## 2.0.0 - 2026-09-22（本地开发版本）

- DSH 看板接入右侧栏“开始”页，入口位于文件与终端之后，仅在项目已初始化时显示；四页界面采用 DeepSeek Harness 原生字体与主题、圆角按钮、悬停说明和自动隐藏滚动条。操作跟随侧栏会话，项目变化和关闭设置会使旧预览失效；补充真实官方侧栏插槽集成测试。
- 新增宿主无关运行内核、CLI、JSON 契约和任务 profile，记录实际命令与退出码、输入/代码/输出哈希、产物及结论证据。
- 新增独立审查前置关系、回执快照校验、文件增删改失效、项目锁、原子状态、迁移备份与可预览恢复快照。
- DSH 改为共享内核适配器，修正会话隔离、宿主接口、配置和完成判定；增加能力、审核、运行、证据、预览与恢复入口。
- 三角色按任务范围和证据覆盖工作，图数/篇幅建议与有来源的硬规则分离，Word/LaTeX 可独立选择。
- 修复机器学习样例数据泄漏与测试集调参，新增算法卡片及优化/预测/评价数值基准。
- 检索后端区分失败/空结果/部分成功，元数据匹配不冒充原文支持。
- 新增依赖组、真实环境诊断、Windows 路径回归修正、CI、完整本地分发与独立 wheel 安装验证。
- 新增来源及分发策略，未核实授权的组件不会自动获得公开发行许可。

2.0 状态/回执存在有意的不兼容变更：旧版先备份，缺乏可靠身份与快照的 PASS 需重新审核。实际验证边界见 docs/upgrade-verification.md。
...
- 符号必须实际排版为标准数学格式（Word 原生 OMML / LaTeX 数学模式），最终论文不得显示 `x_ij`、`alpha`、`C_total`、下划线、反斜杠或公式定界符等源码；符号遵循领域惯例（变量斜体、向量粗体小写、矩阵粗体大写、集合花体、函数名正体），一符一义，单位统一写法、无单位写“无量纲”。
- `术语表格.md` 保持原多列结构（中文/英文术语、定义、首次出现、近义词），仅吸收上述符号书写规范；论文的符号说明表从该表提取“符号、含义、单位”三列，不另建第二份符号定义。
- 新增附录规范：正文保留回答题目所需的模型、公式、结果与验证；附录只放过长推导、补充数据、扩展验证或官方明确要求的内容，每个附录单一主题并与正文双向引用。官方或用户要求附完整代码时，必须写入与最终结果一致的真实完整可运行源码及全部项目内依赖，禁止用说明文字、伪代码、截图、“代码见附件”、省略号、`pass`、待实现占位注释或缺失函数替代。
- 论文结果图要求由“每子问题至少 1 流程图 + 1 结果图”放宽为“每子问题至少 1 幅正式结果图”。
- `tools/figure` 范围由“不做示意图、流程图、架构图”改为额外覆盖建模流程图，新增 `references/chart-types/flowchart.md`：节点形状与语义、实线有向箭头、连续编号、生成路线（优先复用现有生成器，否则用已安装的 Matplotlib/MATLAB，不新增依赖）、SVG 与 ≥300 DPI PNG 成对导出及自检清单。
- 建模流程图提升为**必须交付物**：编程手必须至少生成 1 幅总体建模流程图（覆盖输入、子问题依赖、模型、求解、验证与输出），子问题存在独立分支、循环或三步以上求解链时补充子问题流程图；流程图与三类候选图分开计数，不替代结果图。
- 编程手固定产物、执行顺序、工作流程、质检清单与 `P2` 门禁接入流程图职责；论文手侧负责按编程手产出的流程图入文并核对编号、题注与正文引用（`W1`/`W2` 与自审框架同步）。
- 同步 dsh-plugin 内置知识库。
...
- Y 轴改为按累计 Star 动态选取 1/2/5×10^n 刻度间隔，刻度数稳定在约 7-8 条，避免 Star 增多后在固定图高内 Y 轴过于拥挤。
- X 轴月份刻度保证显示最新月份，且绘图范围延伸到数据更新日期，修复数据滞后时到 9 月仍显示 8 月的问题。
- 同步更新 star_history 测试断言，并新增动态 Y 轴间隔、X 轴含当前月两个用例。

### 修复论文混入内部流程术语

- 论文手 SKILL.md 新增「内容边界（不得写入论文）」声明：门禁（M1/P1/P2/W1/W2）、Subagent 质检、质检简报/回执、证据大纲、复现清单等内部管控信息严禁进入论文正文/摘要/结论/附录。
- 自审框架新增检查项：论文不含内部流程术语。
- paper_format.py 新增内部流程术语残留检测，论文含「门禁/Subagent/质检/复现清单」等词即拦截并阻止发布；配套回归测试。
- 同步至 dsh-plugin 内置知识库。

### 增强截止时间与可提交版本保护

- 新增通用交付与截止时间协议：核验当届官方截止时间与提交限制，尽早验证代码到最终文件的完整链路，并把转换、人工检查、压缩和上传纳入正式排期。
- 引入 Checkpoint V1、Checkpoint V2、内容冻结和最终导出状态；V1 形成后必须独立保留，临近截止时间默认拒绝会使现有可提交版本失效的高风险修改。
- 补充 Word Markdown 格式残留、支撑材料完整性、压缩包可用性和人工上传确认检查，并接入论文手流程、自审与 `W2` 终检。
