...
**运行预算**：耗时取决于算法、样本量、特征维度、调参次数和硬件；先测小实例，再评估全量预算，不作通用秒级承诺。KNN 预测与核 SVM 的规模限制需要单独测量。
...
def tune_max_depth(X_train, y_train, max_depths=range(1, 11)):
    """仅在训练集内部选深度；最终测试集不参与调参。"""
...
# 参数冻结后另行调用 search.score(X_test, y_test)，仅作最终评估。
...
def tune_k(X_train, y_train, k_range=range(1, 21)):
    ...
    search = GridSearchCV(
        pipeline, {'knn__n_neighbors': list(k_range)},
        cv=StratifiedKFold(5, shuffle=True, random_state=42),
        scoring='accuracy', error_score='raise',
    )
    return search.fit(X_train, y_train)

# 确认各训练折样本数不少于最大 K；最终只在冻结后评估测试集。
