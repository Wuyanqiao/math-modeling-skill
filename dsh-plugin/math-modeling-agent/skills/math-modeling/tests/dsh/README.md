...
普通适配器回归共 23 项，不安装 Node 依赖，真实 CLI 用 PATH 中的 Python：
...
Windows 浏览器测试使用本机 Edge；其他平台需为隔离 Playwright 准备 Chromium。`DSH_UI_SCREENSHOT` 与 `DSH_CHECKPOINT_SCREENSHOT` 可分别指定概览及恢复预览截图绝对路径。完整测试共 29 项，其中 3 项官方宿主测试、1 项 React 浏览器测试和 1 项官方侧栏渲染测试需要隔离宿主依赖。没有 `DSH_TEST_NODE_MODULES` 时，这 5 项明确跳过，不被计为通过；运行全部文件的预期计数是 24 通过、5 跳过。
...
| 层次 | 实际覆盖 | 不覆盖 |
|---|---|---|
| adapter.test.mjs（23 项） | 编码与注入边界、长配置 stdin、拒绝/取消/截断、宿主 standing policy、多会话与自定义根、实时预设投影、幂等初始化、旧项目要求迁移与拒权、分块顺序/取消清理、设置、完整 Skill、真实 Python 运行→证据→P1 审查→失效、运行日志、文件恢复与预览版本冲突 | 宿主服务由模拟提供；科学结论仍由独立审核判断 |
...
本次执行环境：Windows、Node 24.11.1、Python 3.13.13；官方 DSH 0.1.7-alpha.1。本机已配置隔离宿主依赖的最终回归结果为 **29/29 通过，0 失败，0 跳过**，耗时 33.7574 秒。完整源码仓库中的记录为 `project-review/logs/upgrade-2.1-dsh.log`，该审计日志不随功能安装包分发。这项结果包括上述 fixture 边界，不表示完成整个桌面 GUI 或真实模型对话测试，也不证明所有平台均已通过。
