...
2.1.2 在本机 Windows / Python 3.13.13 / Node 24.11.1 完整运行 **31/31 通过，0 失败、0 跳过，37.3497 秒**，包含真实预设挂载与 Windows 沙箱专项。记录为 `project-review/logs/fix-2.1.2-dsh.log`；下文保留各用例边界与 2.1.1 历史结果。
...
CI 的 `preset-host` 作业在 Ubuntu 和 Windows 执行真实预设注册回归，Windows 另准备 Python 3.13 并执行此沙箱检查。两个测试步骤都验证实际执行数与零跳过；Linux 不调度 Windows 沙箱用例，避免把平台跳过计为通过。
